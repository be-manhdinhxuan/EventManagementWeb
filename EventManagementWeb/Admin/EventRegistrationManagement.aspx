<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="EventRegistrationManagement.aspx.cs" Inherits="EventManagementWeb.Admin.EventRegistrationManagement" %>
...
<head runat="server">
    ...
    <link href="../Assets/css/pages/event-registration-management.css" rel="stylesheet" />
</head>
<body>
    <form id="form1" runat="server">
        <div class="parent">
            ...
            <main class="main">
                ...
                <h1 class="main__title">Đăng ký - <%= EventTitle %></h1>
                <div class="registration__info"><%= CurrentRegistrations %> / <%= MaxCapacity %> người đăng ký</div>

                <section class="registration-filter" aria-label="Bộ lọc quản lý đăng ký sự kiện">
                    <div class="registration-filter__search">
                        <label class="registration-filter__label">Tìm kiếm nhân viên</label>
                        <div class="registration-filter__search-wrapper">
                            <svg class="registration-filter__search-icon" width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <input type="text" name="txtSearch" class="registration-filter__search-input"
                                placeholder="Tìm kiếm theo tên hoặc email..." value="<%= SearchTerm %>" />
                        </div>
                    </div>
                    <div class="registration-filter__field">
                        <label class="registration-filter__label">Trạng thái:</label>
                        <div class="registration-filter__select-wrapper">
                            <select name="ddlStatus" class="registration-filter__select" onchange="this.form.submit()">
                                <option value="ALL" <%= SelectedStatus == "ALL" ? "selected" : "" %>>Tất cả</option>
                                <option value="Approved" <%= SelectedStatus == "Approved" ? "selected" : "" %>>Xác nhận</option>
                            ...
                            <svg class="registration-filter__select-arrow" width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                <path d="M1 1.5L6 6.5L11 1.5" stroke="#6B7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        </div>
                    </div>

                    <button type="submit" name="btnAction" value="reset" class="registration-filter__reset">
                        <svg class="registration-filter__reset-icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="table" role="table" aria-label="Danh sách đăng ký sự kiện">
                    <div class="table__header" role="row">
                        ...
                        <div class="table__header-cell" role="columnheader">Thời gian đăng ký</div>
                        <div class="table__header-cell" role="columnheader">Trạng thái</div>
                        <div class="table__header-cell" role="columnheader">Ghi chú</div>
                    </div>
                    <div class="table__body">
                        <% if (RegistrationList.Rows.Count == 0)
                            { %>
                        <div class="table__row table__row--empty">
                            <p>Chưa có người đăng ký nào phù hợp với bộ lọc.</p>
                        </div>
                        <% }
                        else
                        { %>
                        <% foreach (DataRow row in RegistrationList.Rows)
                            { %>
                        <div class="table__row" role="row">
                            <div class="table__cell" role="cell">
                                ...
                                <div class="table__registration-name"><%= row["FullName"] %></div>
                            </div>
                            <div class="table__cell" role="cell"><%= row["Email"] %></div>
                            <div class="table__cell" role="cell"><%= row["Phone"] ?? "-" %></div>
                            <div class="table__cell" role="cell"><%= Convert.ToDateTime(row["UpdatedAt"]).ToString("dd-MM-yyyy HH:mm") %></div>
                            <div class="table__cell" role="cell">
                                <span class="badge <%= GetStatusClass(row["Status"].ToString()) %>">
                                    <%= GetStatusText(row["Status"].ToString()) %>
                                </span>
                            </div>
                            <div class="table__cell table__note" role="cell"><%= row["Note"] ?? "-" %></div>
                        </div>
                        <% } %>
                        <% } %>
                    </div>
                ...
                <!-- Registration Cards -->
                <aside class="cards-panel" aria-label="Danh sách đăng ký dạng thẻ">
                    <% foreach (DataRow row in RegistrationList.Rows)
                        { %>
                    <article class="registration-card">
                        <div class="registration-card__header">
                            <div class="registration-card__avatar">
                                <img src="<%= GetUserAvatar(row["Avatar"]) %>" alt="<%= row["FullName"] %>" class="registration-card__image">
                                <div class="registration-card__info">
                                    <h3 class="registration-card__name"><%= row["FullName"] %></h3>
                                    <p class="registration-card__email"><%= row["Email"] %></p>
                                </div>
                            </div>
                            <span class="badge <%= GetStatusClass(row["Status"].ToString()) %>">
                                <%= GetStatusText(row["Status"].ToString()) %>
                        ...
                        <div class="registration-card__meta">
                            <div class="registration-card__meta-item">
                                <p class="registration-card__meta-label">Số điện thoại:</p>
                                <span><%= row["Phone"] ?? "-" %></span>
                            </div>
                            <div class="registration-card__meta-item">
                                <p class="registration-card__meta-label">Thời gian đăng ký:</p>
                                <span><%= Convert.ToDateTime(row["UpdatedAt"]).ToString("dd-MM-yyyy HH:mm") %></span>
                            </div>
                            <div class="registration-card__meta-item">
                                <p class="registration-card__meta-label">Ghi chú:</p>
                                <span><%= row["Note"] ?? "-" %></span>
                            </div>
                        </div>
                    </article>
                ...
                <nav class="pagination" aria-label="Phân trang">
                    <div class="pagination__info">Hiển thị <%= (CurrentPage - 1) * 10 + 1 %>-<%= Math.Min(CurrentPage * 10, TotalRecords) %> trong <%= TotalRecords %> kết quả</div>
